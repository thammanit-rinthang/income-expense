...
export default function CombinedDashboard() {
  ...
  const bonPercent = combinedIncome > 0 ? Math.min((Number(summary.person_a_spent) / combinedIncome) * 100, 100) : 0;
  const rayPercent = combinedIncome > 0 ? Math.min((Number(summary.person_b_spent) / combinedIncome) * 100, 100) : 0;

  return (
    <div className="space-y-6">
      {/* Combined Stats */}
      <div className="grid grid-cols-1 gap-4">
        <div className="rounded-[1.5rem] bg-primary p-6 text-primary-content shadow-xl shadow-primary/15">
          <p className="text-sm font-bold opacity-75 mb-1">เงินใช้ได้รวม</p>
          <p className="finance-display font-black">
            {formatCurrency(Number(bon.remaining) + Number(ray.remaining))}
          </p>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div className="finance-card p-5 flex items-center gap-3">
            <div className="p-2 bg-success/10 text-success rounded-xl"><Wallet size={20} /></div>
            <div>
              <p className="finance-label">รายรับรวม</p>
          ...
          <div className="finance-card p-5 flex items-center gap-3">
            ...
            <div>
              <p className="finance-label">จ่ายประจำแล้ว</p>
      ...
      <div className="finance-card p-6 space-y-6">
        <div className="flex items-center gap-2 px-1">
          ...
          <h3 className="font-black text-gray-900 tracking-tight">เทียบยอดใช้จริง</h3>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="finance-card bg-gray-50 p-4 text-center">
          <p className="finance-label mb-1">Bon เหลือใช้</p>
          <p className="font-black text-primary">{formatCurrency(bon.remaining)}</p>
        </div>
        <div className="finance-card bg-gray-50 p-4 text-center">
          <p className="finance-label mb-1">Ray เหลือใช้</p>
